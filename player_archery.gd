## archery.gd
## Spawns player_archery units periodically up to a cap.
## Pauses during "slack" wave mode unless enemy enters proximity.
extends Area2D

@export var archer_scene: PackedScene = preload("res://entities/ally/player_archer.tscn")
@export var spawn_interval: float = 8.0
@export var max_active_archers: int = 6
...
func _try_spawn() -> void:
	if _destroyed:
		return
	if get_tree().get_nodes_in_group("player_archers").size() >= max_active_archers:
		return
	if not archer_scene:
		push_error("Archery: archer_scene not set!")
		return
	var k := archer_scene.instantiate()
